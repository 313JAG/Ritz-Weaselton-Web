...
function buildCodeEntry(row, category) {
  ...
  if (!code) return null
  const rateGroup = RATE_CODE_GROUPS.find((group) => group.codes.includes(code))

  const company = row.company || row.university || row.name || row.status || 'Unknown'
  ...
  return {
    ...
    rateGroupName: rateGroup?.name || null,
...
const HIDDEN_DERIVED_CATEGORIES = new Set(['Public / Membership Codes (No Corporate Affiliation Needed)', 'Marriott Internal / Employee Codes', 'Promo & Package Codes (From FlyerTalk Wiki)'])
...
function parseCodesFile() {
  ...
  const derivedCategoryPresets = [...categoryCodes.entries()].filter(([category, codesForCategory]) => codesForCategory.length > 0 && !HIDDEN_DERIVED_CATEGORIES.has(category)).map(([category, codesForCategory]) => categoryPreset(category, codesForCategory))

  return {
    codes,
    presets: dedupePresets([...DEFAULT_PRESETS.map((preset) => ({ ...preset })), ...derivedCategoryPresets]),
  }
}
