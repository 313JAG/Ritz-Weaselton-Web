...
function fallback(query: string): Choice[] {
  return DESTINATIONS.filter(([city]) => city.toLowerCase().includes(query.toLowerCase()))
    .slice(0, 6)
    .map(([city, country]) => ({ label: `${city}, ${country}`, city, country, searchTerm: city }))
}
...
export function DestinationPicker({
  ...
  async function find(next: string) {
    ...
    try {
      ...
      const deduped = [...placeChoices, ...autocompleteChoices].filter(
        (choice, index, all) =>
  ...
  async function select(choice: Choice) {
    ...
    try {
      ...
      /* autocomplete already provides a usable destination */
    }
  ...
  const freeTextChoice: Choice | null =
    query.trim().length >= 2 && !loading && !hasSelected && choices.length === 0
  ...
  return (
    <div className="relative">
      ...
      <div className="relative">
        <MapPinIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
        <Input
          className="h-14 pl-10 pr-10 text-base md:text-lg"
          id="city"
          onChange={(event) => void find(event.target.value)}
          placeholder="City, landmark, airport, or neighbourhood"
          value={query}
        />
        {loading ? (
          <SpinnerGapIcon className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin text-muted-foreground" />
        ) : null}
      </div>
      {choices.length || freeTextChoice ? (
        <div className="absolute z-20 mt-2 grid w-full overflow-hidden rounded-xl border bg-popover p-1 shadow-xl">
          {choices.map((choice) => (
            <Button
              className="justify-start whitespace-normal px-3 py-3 text-left"
              key={`${choice.label}-${choice.city}`}
          ...
          {freeTextChoice ? (
            <Button
              className="justify-start whitespace-normal px-3 py-3 text-left"
              onClick={() => void select(freeTextChoice)}
              type="button"
              variant="ghost"
            >
              <MapPinIcon className="shrink-0" />
              {freeTextChoice.label}
            </Button>
          ) : null}
        </div>
      ) : null}
