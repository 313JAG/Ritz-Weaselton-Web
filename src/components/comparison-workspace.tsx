...
export function ComparisonWorkspace({
  ...
  const activeBestRate = active?.rates.find((rate) => rate.code === active.bestCode) || null
  const stayNights = Math.max(1, Math.round((new Date(`${job?.params.checkOut || ""}T12:00:00`).getTime() - new Date(`${job?.params.checkIn || ""}T12:00:00`).getTime()) / 86400000) || 1)
      ...
      <div className="rw-comparison-grid">
        ...
        <article className="rw-rate-detail">
          {active ? <>
            ...
            <div className="rw-price-hero"><div><span>Marriott rate / night</span><strong>{formatCurrency(active.bestPrice, active.currency)}</strong><em>{active.bestCodeLabel || "No code"} · {active.currency || "Marriott"} booking quote</em></div><div><span>Marriott stay total</span><strong>{formatCurrency(activeBestRate?.totalPrice ?? null, active.currency)}</strong><em>{stayNights} {stayNights === 1 ? "night" : "nights"}</em></div><div><span>Standard rate / night</span><strong>{formatCurrency(active.baselinePrice, active.currency)}</strong></div><div className="is-saving"><span>You save / night</span><strong>{formatCurrency(active.savings, active.currency)}</strong></div></div>
            <div className="rw-matrix-head"><div><strong>Every checked Marriott rate</strong><span>Matches Marriott’s advertised rate quote. Taxes and fees are confirmed at checkout.</span></div><Button onClick={onToggleUnavailable} size="sm" variant="outline">{showUnavailableRates ? "Available only" : "Show unavailable"}</Button></div>
            <div className="rw-rate-matrix"><Table><TableHeader><TableRow><TableHead>Code</TableHead><TableHead>Rate / night ({active.currency || "currency"})</TableHead><TableHead>vs standard</TableHead><TableHead>Availability</TableHead><TableHead /></TableRow></TableHeader><TableBody>{active.rates.filter((rate) => showUnavailableRates || rate.available).map((rate) => <TableRow className={rate.code === active.bestCode ? "rw-winning-rate" : ""} key={rate.code}><TableCell><strong>{rate.label}</strong><small>{rate.company}</small></TableCell><TableCell>{rate.available ? <><strong>{formatCurrency(rate.price, rate.currency)}</strong><small>{formatCurrency(rate.totalPrice ?? null, rate.currency)} stay total</small></> : "—"}</TableCell><TableCell>{rate.available && active.baselinePrice !== null && rate.price !== null ? formatCurrency(Math.max(active.baselinePrice - rate.price, 0), rate.currency) : "—"}</TableCell><TableCell>{rate.available ? rate.code === active.bestCode ? "Best rate" : "Available" : rate.error || "Unavailable"}</TableCell><TableCell>{rate.bookingUrl && rate.available ? <Button asChild size="sm"><a href={rate.bookingUrl} rel="noreferrer" target="_blank">Book</a></Button> : null}</TableCell></TableRow>)}</TableBody></Table></div>
