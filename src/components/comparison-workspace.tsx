      ...
      <div className="rw-comparison-grid">
        ...
        <article className="rw-rate-detail">
          {active ? <>
            ...
            <div className="rw-price-hero"><div><span>All-in nightly rate</span><strong>{formatCurrency(active.bestPrice, active.currency)}</strong><em>{active.bestCodeLabel || "No code"} · taxes and fees returned by Marriott</em></div><div><span>All-in stay total</span><strong>{formatCurrency(activeBestRate?.totalPrice ?? null, active.currency)}</strong><em>{stayNights} {stayNights === 1 ? "night" : "nights"}</em></div><div><span>Standard all-in rate</span><strong>{formatCurrency(active.baselinePrice, active.currency)}</strong></div><div className="is-saving"><span>You save / night</span><strong>{formatCurrency(active.savings, active.currency)}</strong></div></div>
            <div className="rw-matrix-head"><div><strong>Every checked all-in rate</strong><span>Nightly totals include the taxes and fees returned by Marriott. Confirm final checkout terms before booking.</span></div><Button onClick={onToggleUnavailable} size="sm" variant="outline">{showUnavailableRates ? "Available only" : "Show unavailable"}</Button></div>
            <div className="rw-rate-matrix"><Table><TableHeader><TableRow><TableHead>Code</TableHead><TableHead>All-in / night</TableHead><TableHead>vs standard</TableHead><TableHead>Availability</TableHead><TableHead /></TableRow></TableHeader><TableBody>{active.rates.filter((rate) => showUnavailableRates || rate.available).map((rate) => <TableRow className={rate.code === active.bestCode ? "rw-winning-rate" : ""} key={rate.code}><TableCell><strong>{rate.label}</strong><small>{rate.company}</small></TableCell><TableCell>{rate.available ? <><strong>{formatCurrency(rate.price, rate.currency)}</strong><small>{formatCurrency(rate.totalPrice ?? null, rate.currency)} stay total</small></> : "—"}</TableCell><TableCell>{rate.available && active.baselinePrice !== null && rate.price !== null ? formatCurrency(Math.max(active.baselinePrice - rate.price, 0), rate.currency) : "—"}</TableCell><TableCell>{rate.available ? rate.code === active.bestCode ? "Best rate" : "Available" : rate.error || "Unavailable"}</TableCell><TableCell>{rate.bookingUrl && rate.available ? <Button asChild size="sm"><a href={rate.bookingUrl} rel="noreferrer" target="_blank">Book</a></Button> : null}</TableCell></TableRow>)}</TableBody></Table></div>
